...
> Replace `<username>` below with the Docker Hub account hosting the image.

## Run

```bash
# Pull the pinned image
docker pull hamzaibrahim21/so3lr-sf:v1.0

# Interactive shell
docker run --rm -it hamzaibrahim21/so3lr-sf:v1.0

# Run a calculation (mount your input/output dir, and -w into it)
docker run --rm -v "$PWD/data:/data" -w /data \
  hamzaibrahim21/so3lr-sf:v1.0 \
...
```bash
docker build -f Docker/Dockerfile -t <username>/so3lr-sf:<tag-name> .
docker push <username>/so3lr-sf:<tag-name>
```
