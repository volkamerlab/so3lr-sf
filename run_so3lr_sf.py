...
def main():
    ...
    try:
        ...
        output_dir = setup_output_directory(protein_path, args)
        ...
        if args.trim:
            working_protein_path = perform_trimming(protein_path, args, output_dir, logger)

        # Step 2: Protein optimization
        if args.optimize:
            logger.info("=== OPTIMIZATION PHASE ===")
            working_protein_path = optimize_protein(working_protein_path, calc, args, output_dir, optimization_log, logger)
